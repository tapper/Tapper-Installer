package Artemis::Installer::Precondition::Image;
...
Artemis::Installer::Precondition::Package - Install a package to a given location
...
method copy_menu_lst()
{
        ...
        my $artemis_conf  = $self->cfg->{paths}{grubpath};

        return $self->log_and_exec("cp $menu_lst_file $artemis_conf/$hostname.lst");
};
        ...
        my $entry                = "title Test run $id\n".
          "root (hd$grub_device,$partition_number)\n".
            "kernel /boot/vmlinuz root=$device_file earlyprintk=serial,ttyS0,115200 ".
              "console=ttyS0,115200 ip=dhcp noapic artemis_host=".$self->cfg->{server}."\n";
